...
/**
 * Build a `{ sport_id: [type, ...] }` catalog from the user's existing
 ...
 * is small (one row per distinct (sport_id, type)), no need to paginate.
 */
export async function buildTypeSuggestionsBySportId(
  userId: number,

...
export default async function NewEventPage() {
  ...
  // Distinct (activity, type) pairs feed the autocomplete on the Type input.
  // Keeping it server-side avoids a round-trip when the activity selector
  // changes; one query at single-user scale is negligible.
  const typesByActivity = await buildTypeSuggestionsByActivityId(user.id);
  ...
  return (
    <div className="max-w-[640px]">
      ...
      <NewEventForm activities={activitiesList} typesByActivity={typesByActivity} />
    </div>
  );
}

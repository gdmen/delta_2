...
/**
 * Bulk-merge for one source/activity group on /data/duplicates. Headline is
...
async function seedEvent(
  ...
  const [e] = await ctx
    .getDb()
    .insert(events)
    .values({ userId: 1, activityId, type: "Ride", startedAt, source, durationMinutes, status: "visible" })
...
describe("POST /api/events/duplicates/bulk-merge", () => {
  it("BM1: a 2-event pair merges into one composite with the chosen activity + max duration", async () => {
    const stravaRide = await seedActivity("strava:Ride");
    const whoopRide = await seedActivity("whoop:Ride");
    const cleanRide = await seedActivity("Ride");
    ...
    const res = await POST(
      req({
        group: { sourceA: "strava", activityIdA: stravaRide, sourceB: "whoop", activityIdB: whoopRide },
        activityId: cleanRide,
      }),
    );
    ...
    expect(composite?.activityId).toBe(cleanRide);
  ...
  it("BM3: a group matching no pairs merges nothing", async () => {
    ...
    const res = await POST(
      req({
        group: { sourceA: "strava", activityIdA: stravaRide, sourceB: "whoop", activityIdB: 9999 },
        activityId: stravaRide,
      }),
    );
  ...
  it("BM4: malformed body → 400", async () => {
    expect((await POST(req({ group: { sourceA: "x" }, activityId: 1 }))).status).toBe(400);
    expect((await POST(req({ group: { sourceA: "a", activityIdA: 1, sourceB: "b", activityIdB: 2 } }))).status).toBe(400);
  });

  it("BM5: unowned composite activity → 400, nothing merged", async () => {
    const stravaRide = await seedActivity("strava:Ride");
    const whoopRide = await seedActivity("whoop:Ride");

    ...
    <div className="max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-0 border border-border rounded overflow-hidden">
            {BJJ_TYPES.map((t) => (
              <button
                key={t.value}
                type="button"
                onClick={() => setType(t.value)}
                className={`py-3 text-[0.875rem] font-medium transition-colors ${
        ...
        <div className="flex items-center gap-4">
          ...
          {savedAt && (
            <span className="text-[0.8125rem] text-accent-green">✓ Saved</span>
          )}
        </div>
      </form>
    </div>

...
export function WipeSourceButton({ source }: { source: string }) {
  ...
  return (
    <section className="mt-10 pt-6 border-t border-border">
      ...
      {/* Modal lives in its own component so its state is fresh on every
          open — avoids resetting state via setState-in-effect, which the
          react-hooks/set-state-in-effect lint rule (correctly) flags as
          a cascading-render hazard. */}
      {open && (
        <WipeModal source={source} onClose={() => setOpen(false)} />
      )}
    </section>
  );
}

function WipeModal({ source, onClose }: { source: string; onClose: () => void }) {
  ...
  // Fetch counts on mount. The component remounts every time the modal
  // opens (parent gates on `open`), so this effect runs fresh each time
  // without resetting state synchronously.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
